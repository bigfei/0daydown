...
BaiduPan.prototype.inputSharePass = function(panUrl, pass, articleUrl, o) {
  var driver = this.driver;
  return getPageSource(panUrl, driver).then(function($) {
    var isErrorOccurred = function() {
      var errorOccur = false;
      if ($('#share_nofound_des').length) {
        console.log("Pan sharing of " + panUrl + " with pass( " + pass + " ) in articleUrl( " + articleUrl + " ) are not available anymore.");
        o.status.scraped_status = 'PANFILE_SHARING_CANCEL';
        errorOccur = true;
      }

      if ($('body.error-404').length) {
        console.log("Pan sharing of " + panUrl + " with pass( " + pass + " ) in articleUrl( " + articleUrl + " ) is not found(404 error).");
        o.status.scraped_status = '404_ERROR';
        errorOccur = true;
      }
      return errorOccur;
    }
    ...
    if (pass) {
      if (!isErrorOccurred()) {
        driver.wait(until.elementLocated(by.id('accessCode')), 10 * 1000).then(function(elem) {
          elem.sendKeys(pass);
        ...
        })//end of driver.wait.then
      };//end of setScrapedStatus
    } else {
      isErrorOccurred();
    }
  });
...
BaiduPan.prototype.checkBaiduPan = function(panUrl, pass) {
  ...
  return driver.wait(until.elementLocated(by.css('div.bd-aside')), 10 * 1000).then(function() {
    return driver.getPageSource().then(function(src) {
...
BaiduPan.prototype.checkBaiduPanByApi = function(panUrl, pass, articleUrl, o) {
  var driver = this.driver;
  this.inputSharePass(panUrl, pass, articleUrl, o);
  return driver.wait(until.elementLocated(by.css('div.bd-aside,#vers-update-new,body.error-404')), 10000).then(function() {
    driver.sleep(700);
    return driver.executeScript('return window.yunData ? yunData : ""').then(function(data) {
      if (data && data.FILEINFO) {
        ...
        o.status.scraped_status = 'OK';
        return {files: files, uk: data.SHARE_UK, shareid: data.SHARE_ID}
      } else {
        o.status.scraped_status || (o.status.scraped_status = "YUNDATA_MISSING");
      ...
      console.warn(err);
      driver.navigate().refresh();
      this.checkBaiduPanByApi(panUrl, pass, articleUrl, o);
    }.bind(this));
  }.bind(this), function(err) {
