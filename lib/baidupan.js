...
BaiduPan.prototype.inputSharePass = function(panUrl, pass) {
  var driver = this.driver;
  driver.get(panUrl);
  if (pass) {
...
BaiduPan.prototype.checkBaiduPanByApi = function(panUrl, pass) {
  var driver = this.driver;
  this.inputSharePass(panUrl, pass);
  return driver.wait(until.elementLocated(by.css('div.bd-aside,#vers-update-new,div.module-error')), 10000).then(function() {
    driver.sleep(700);
    return driver.executeScript('return yunData').then(function(data) {
      ...
      console.log(err);
      driver.navigate().refresh();
      this.checkBaiduPanByApi(panUrl, pass);
    });
  }.bind(this), function(err) {
    console.log(err);
    driver.navigate().refresh();
